...
export default function CombinedNavbar() {
  ...
  return (
    <>
      {/* 1. TOP NAVBAR - CHARCOAL & WHITISH BORDER */}
       <AnimatePresence>
  {!scrolled && (
    <motion.nav
    ...
    >
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* TOP HOVER POPUP CARD (Desktop Only) */}
              <AnimatePresence>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/10">
                        ...
                        <span className={`${poppins.className} text-[10px] font-medium text-neutral-300 bg-white/10 px-1.5 py-0.5 rounded border border-white/15`}>
                          {item.meta}
                        </span>
                      </div>
      ...
      >
        {mobileMenuOpen ? <HiOutlineXMark size={18} /> : <HiBars3 size={18} />}
      </button>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8, scale: 0.95 }}
            transition={{ duration: 0.18, ease: "easeOut" }}
            className="absolute top-[calc(100%+6px)] left-0 right-0 w-full bg-neutral-900/98 border border-white/20 rounded-2xl p-2.5 backdrop-blur-2xl shadow-2xl md:hidden overflow-hidden z-50"
          >
            <div className="flex flex-col gap-1.5">
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon className="text-white mt-0.5 shrink-0" size={15} />
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className={`${poppins.className} text-xs font-semibold text-white`}>
                          {item.name}
                        </span>
                        <span className={`${poppins.className} text-[8px] font-medium text-neutral-300 bg-white/10 px-1 py-0.5 rounded border border-white/15`}>
                          {item.meta}
                        </span>
                      </div>
      ...
      {/* <AnimatePresence>
        {!scrolled && mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-24 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-sm rounded-2xl bg-neutral-900 border border-white/15 p-4 md:hidden shadow-xl"
          >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 rounded-lg bg-neutral-800 border border-white/10 text-red-400 group-hover:bg-red-600 group-hover:text-white transition-colors">
                        <Icon size={18} />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <p className={`${poppins.className} text-sm font-semibold text-white`}>{item.name}</p>
                          <span className={`${poppins.className} text-[9px] font-semibold text-red-400 bg-red-950/60 px-1.5 py-0.5 rounded border border-red-900/40`}>
                            {item.meta}
                          </span>
                        </div>
                        <p className={`${poppins.className} text-xs text-neutral-400`}>{item.desc}</p>
                      </div>
                    </div>
                    <HiChevronRight className="text-neutral-500 group-hover:text-white transition-colors" size={16} />
                  </a>
                );
              })}
            </div>
  ...
  {scrolled && (
    <motion.div
      ...
      onMouseLeave={() => {
        setIsOpen(false);
        setHoveredItem(null);
        if (typeof lastIndexRef !== "undefined" && lastIndexRef.current) {
          lastIndexRef.current = null;
        }
      }}
    >
      ...
      {/* SHARED TOOLTIP - MONOCHROME BLACK & WHITE GLASS */}
      <AnimatePresence>
        {isOpen && hoveredItem && hoveredData && (
          <motion.div
            ...
            className="absolute bottom-6 left-1/2 -ml-[52px] z-40 pointer-events-none"
          >
            <div className="min-w-[104px] rounded-xl border border-white/20 bg-black/90 px-4 py-2 text-center shadow-[0_10px_25px_rgba(0,0,0,0.95)] backdrop-blur-xl">
              <div className="relative h-4 overflow-hidden flex items-center justify-center">
                <AnimatePresence mode="popLayout" custom={direction}>
                  <motion.span
                    ...
                    className={`${poppins.className} text-[11px] font-semibold text-white tracking-widest uppercase whitespace-nowrap`}
                  >
                    {hoveredData.name}
                  </motion.span>
                </AnimatePresence>
            ...
            <div className="absolute left-1/2 -bottom-1 h-2 w-2 -translate-x-1/2 rotate-45 border-b border-r border-white/20 bg-black" />
          </motion.div>
        )}
      </AnimatePresence>

      {/* ARC ICONS - HOVER & CLICK COMPATIBLE */}
      <AnimatePresence>
        {isOpen && (
      ...
      {/* TRIGGER BUTTON - DUAL HOVER & CLICK */}
      <motion.button
        type="button"
